...
C    Y BECOMES THE THICKNESS OF THE PLANE STRAIN
		CALL MAT1_MAT2YZ(NTENS,STRAN,TSTRAIN,HALF)
		CALL MAT1_MAT2YZ(NTENS,DSTRAN,DTSTRAIN,HALF)
		CALL MAT1_MAT2YZ(NTENS,STRESS,TSTRESS,ONE)
...
C     PHASES P TENSOR
		CALL PTENSOR(AINC,CINC,BMM,SMM,PINI)
...
C         CALCULATE DAMAGE IN MATRIX
     ...
     1 DAMAGEN)
      ...
      END IF	  
C----------------------------------------------------------------
C     OUTPUT CHANGE FROM Z TO Y
      CALL MAT2_MAT1YZ(NTENS,TSTRESS,STRESS,ONE)	 
      CALL MAT4_MAT2YZ(HC,HCMATRIX,1)
      DO I = 1 , NTENS
        DO J = 1 , NTENS
...
C-----------------------------------------------------------------------	 
C
      SUBROUTINE MAT2_MAT1YZ(NTENS,TENSOR,VECTOR,FACT)
C
C ====================================================================
C
C =================== MAT2_MAT1YZ: TENSOR TO VECTOR=====================
C
C ====================================================================
      INCLUDE 'ABA_PARAM.INC'
C
...
C
      RETURN
      END
      SUBROUTINE MAT1_MAT2YZ(NTENS,VECTOR,TENSOR,FACT)
C ====================================================================
C 
C                MAT1_MAT2YZ : 2D PLAIN STRAIN VECTOR TO 3D TENOR Y TO Z
C
C ====================================================================
      INCLUDE 'ABA_PARAM.INC'
C
...
C
      ...
      TENSOR(3 , 3) = VECTOR( 2 )
	  TENSOR(2 , 2) = VECTOR( 3 )
      TENSOR(1 , 3) = VECTOR( 4 )*FACT
      TENSOR(3 , 1) = VECTOR( 4 )*FACT
C
      RETURN
      END
C-----------------------------------------------------------------------
...
C ====================================================================
C
      SUBROUTINE MAT4_MAT2YZ(TENSOR,DMATRIX,ICOE)
      INCLUDE 'ABA_PARAM.INC'
C
      DIMENSION TENSOR(3,3,3,3),DMATRIX(6,6)
C
...
C
      DMATRIX(1,1)=TENSOR(1,1,1,1)
      DMATRIX(1,2)=TENSOR(1,1,3,3)
      DMATRIX(1,3)=TENSOR(1,1,2,2)
      DMATRIX(1,4)=TENSOR(1,1,1,3)*COE1
      DMATRIX(1,5)=TENSOR(1,1,3,2)*COE1
      DMATRIX(1,6)=TENSOR(1,1,1,2)*COE1
C
      DMATRIX(2,1)=TENSOR(3,3,1,1)
      DMATRIX(2,2)=TENSOR(3,3,3,3)
      DMATRIX(2,3)=TENSOR(3,3,2,2)
      DMATRIX(2,4)=TENSOR(3,3,1,3)*COE1
      DMATRIX(2,5)=TENSOR(3,3,3,2)*COE1
      DMATRIX(2,6)=TENSOR(3,3,1,2)*COE1
C
      DMATRIX(3,1)=TENSOR(2,2,1,1)
      DMATRIX(3,2)=TENSOR(2,2,3,3)
      DMATRIX(3,3)=TENSOR(2,2,2,2)
      DMATRIX(3,4)=TENSOR(2,2,1,3)*COE1
      DMATRIX(3,5)=TENSOR(2,2,3,2)*COE1
      DMATRIX(3,6)=TENSOR(2,2,1,2)*COE1
C
      DMATRIX(4,1)=TENSOR(1,3,1,1)*COE1
      DMATRIX(4,2)=TENSOR(1,3,3,3)*COE1
      DMATRIX(4,3)=TENSOR(1,3,2,2)*COE1
      DMATRIX(4,4)=TENSOR(1,3,1,2)*COE2
      DMATRIX(4,5)=TENSOR(1,3,3,2)*COE2
      DMATRIX(4,6)=TENSOR(1,3,1,2)*COE2
C
      DMATRIX(5,1)=TENSOR(3,2,1,1)*COE1
      DMATRIX(5,2)=TENSOR(3,2,3,3)*COE1
      DMATRIX(5,3)=TENSOR(3,2,2,2)*COE1
      DMATRIX(5,4)=TENSOR(3,2,1,3)*COE2
      DMATRIX(5,5)=TENSOR(3,2,3,2)*COE2
      DMATRIX(5,6)=TENSOR(3,2,1,2)*COE2
C
      DMATRIX(6,1)=TENSOR(1,2,1,1)*COE1
      DMATRIX(6,2)=TENSOR(1,2,3,3)*COE1
      DMATRIX(6,3)=TENSOR(1,2,2,2)*COE1
      DMATRIX(6,4)=TENSOR(1,2,1,3)*COE2
      DMATRIX(6,5)=TENSOR(1,2,3,2)*COE2
      DMATRIX(6,6)=TENSOR(1,2,1,2)*COE2
C
C
      RETURN
      END
