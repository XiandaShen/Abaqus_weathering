...
C----------------------------------------------------------------------------
C
C STATE VARIABLE: 1 BIOTITE DIRECTION, 2 WR, 3 DAMAGE, 4 VREV, 5 VSTRAINI, 6 BOITITE LENGTH 7-43 cracks length
...
C    ORIENTATION	  
	  ...
	  THETA(1)=0.0D0*PI/2.0D0+STATEV(1)
	  PSI(1)=0.0D0*PI/2.0D0
...
C----------------------------------------------------------------
C     CRACK PARAMETERS
c	  BCON=(1.0D0-POISONR)/SMM/SQRT(PI)
	  SIGMAC=1000.0D0
	  TOUGHO=0.1D0
C	  
...
C    THE VOLUME OF WEATHERED BIOTITE
		WRCT=10**(-12.32D0)*TIME(2)**(-0.603)
		ROUGHNESS=137.5D0*TIME(2)**(0.1433)
...
C----------------------------------------------------------------  
      TDAMAGE(:,:)=0.0D0
	  DO I=2,IPHASEN-1
       CALL Ai_Bij(DIRECTIONVECTOR(I-1,:),PSTRESS(IPHASEN,:,:),VECT11)
          CALL Ai_Bi(VECT11,DIRECTIONVECTOR(I-1,:),3,TEMP11)
       WRITE(6,*) 'TEMP11',TEMP11
          TEMP12=TEMP11*SIGMAC/TOUGHO/(SIGMAC-TEMP11)
C		 WRITE(6,*) 'TEMP11',TEMP11
	     IF (TEMP12.GT.STATEV(I+5)) THEN
C       WRITE(6,*) 'anew',TEMP11	
	        COMPONENTL(I)=TEMP12
		 END IF
         CALL A_Bij(WEIGHT(I-1)/VREV*COMPONENTL(I)**3,
     1 DIRECTIONTENSOR(I,:,:),3,TEMP21)
C	 		 WRITE(6,*) 'TEMP12',TEMP12,I
         CALL Aij_PLUS_Bij(TDAMAGE,TEMP21,TDAMAGE)		 
        END DO
C       WRITE(6,*) 'TDAMAGE',TDAMAGE
C----------------------------------------------------------------
C     OUTPUT
C----------------------------------------------------------------
...
C
      ...
      TENSOR(3 , 3) = VECTOR( 2 )
	  TENSOR(1 , 1) = VECTOR( 3 )
      TENSOR(3 , 2) = VECTOR( 4 )*FACT
      TENSOR(2 , 3) = VECTOR( 4 )*FACT
C
      RETURN
      END
C-----------------------------------------------------------------------
