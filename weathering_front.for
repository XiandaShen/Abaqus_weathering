...
C FOR CALCULATION
      PARAMETER(INTERVAL=31536.0D3,AB=0.15D0,AR=3.0D0,CINCIN=1.0D0)
C FOR CONSTANT
...
C
C----------------------------------------------------------------
C             SHARING WITH OTHER ELEMENTS AND UEXTERNALDB	    
C----------------------------------------------------------------
	  PARAMETER(NELE=6)
	  COMMON/EACHIN/DEEPEST
	  COMMON/DALABLE/DVECTOR(NELE)
...
C	 
     ...
     1 CINC,AINC,SMM,BMM,SMI,BMI,VSTRAINI,DAMAGE,WTIME
C      
...
C----------------------------------------------------------------------------
C
C STATE VARIABLE: 1 PSI, 2 WR, 3 CINC, 4 DAMAGE, 5 VSTRAINI, 6 AINC, 
C 7 WEAHTERING TIME
	  IF (TIME(2).EQ.0) THEN	
...
C    THE VOLUME OF WEATHERED BIOTITE
		IF (WTIME.EQ.-1.0D0) THEN
		   IF (COORDS(2).GE.(DEEPEST-12.0D0)) THEN
			  WTIME=TIME(2)
		   END IF
		END IF 
		
		WRCT=10**(-12.32D0)*(TIME(2)-WTIME+0.5D0)**(-0.603)
		ROUGHNESS=137.5D0*(TIME(2)-WTIME+0.5D0)**(0.1433)
...
C     PHASES P TENSOR
		...
		CALL TRANSP(TRANSPOSE(Q),PINI,PPI)
...
C      WRITE(6,*) STRESS		
     ...
     1 *CINCIN*AR	  
	  STATEV(7)=WTIME  
      IF (DAMAGE.GT.0.0D0) THEN
	  DVECTOR(NOEL)=COORDS(2)
	  END IF
	  RETURN
	 END
C
